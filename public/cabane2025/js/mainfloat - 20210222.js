...
function total_reservoir(brix) {
    ...
    sirop_total = calcule_brix_uni(brix, volume_total).toFixed(0);
...
//    return "ws://boilerhouse.ddns.net:3001/" + l.pathname + path;
...
// on affiche quand meme  ...because possible appel par la fonction de mise a jour du brix
        ...
        if ((contient >= 0) && (this.liquide === 'C')) {
            contient_gal_sirop = calcule_brix(contient);
...
function calcule_brix(gal_seve) {
    var brix = $("#spinner").spinner('value');  // vachercher le brix dans le spinner
    var ratio = 66;
    return(gal_seve * brix / ratio);
}
