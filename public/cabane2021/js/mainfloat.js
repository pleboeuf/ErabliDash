...
function toggle_rf2() {
    ...
    $(".toggle").on("toggle", function (e, active) {
        if (active) {
            ...
            total_reservoir();
            if (DebugON) console.log("SEVE !");
        } else {
            r["RF2"].changeCouleur(FILTRAT);
            total_reservoir();
            if (DebugON) console.log("FILTRAT !");
        }
    });
}
...
function demarre(draw) {
    ...
    $.getJSON(url, function (data) {
        $.each(data.feed.entry, function (index, value) {
            myComposantes[value.gsx$composante.$t] = new Composante(
                value.gsx$type.$t,
                myNumber(value.gsx$x.$t),
                myNumber(value.gsx$y.$t),
                myNumber(value.gsx$dimx.$t),
        ...
        if (DebugON) console.log("output dans demarre après dessine " + url);
    });

    if (DebugON) console.log("output fin demarre " + url);
}
function myNumber(n) {
    // enleve les espaces et change les , en .  les chiffres dans les spreadsheet google
    var rep = n.replace(/ /g, "");
    rep = rep.replace(/,/g, ".");
    return Number(rep);
}
...
function openSocket() {
    ...
    websocket.onmessage = function (msg) {
        if (DebugON) console.log("Socket onmessage.");
...
function wsUri(path) {
    ...
    if (l.port == 8080)
        return "ws://boilerhouse.ddns.net:3300/" + l.pathname + path; // test sur la prod
...
function oldparseEvent(data) {
    ...
    var m_pumps = [];
    var m_osmoses = [];
    if (DebugON) console.log("myd " + myDevices);
    if (DebugON) console.log("Old " + myDevicesOld + "  res " + res);
    if (res !== undefined && res.length != 0) {
        res.forEach(function (myres) {
            ...
            if (particule) {
                ...
                if (vu) m_vaccuum.push(vu);
                var p = _.find(data.pumps, nom_device);
                if (p) m_pumps.push(p);
                var o = _.find(data.osmose, nom_device);
                if (o) m_osmoses.push(o);
            }
        });
        if (m_devices.length > 0) scandevices(m_devices);

        if (m_pumps.length > 0) scanpumps(m_pumps);
        if (m_tanks.length > 0) scantanks(m_tanks);
        if (m_vaccuum.length > 0) scanvacuum(m_vaccuum);
        if (m_valves.length > 0) scanvalve(m_valves);
        if (m_osmoses.length > 0) scanosmose(m_osmoses);
        updateOsmoseDernMAJ(data.osmose); /// patch pour mettre a jour dern  maj
    }
}
...
function chargelecture() {
    ...
    if (DebugON) console.log("output dans chargelecture AVANT socket");

    openSocket();
    if (DebugON) console.log("output dans chargelecture APRÈS opensocket");
...
function scandevices(arr) {
    arr.forEach(function (no, i) {
        if (no["lastUpdatedAt"]) last_lecture(no["lastUpdatedAt"]);
...
function simple_moving_averager(period) {
    // moyenne qui bouge pour le debit
    var nums = [];
    return function (num) {
        if (num) {
            // hack  si appel sans nombre  retour le resultat
            nums.push(num);
...
function scanpumps(arr) {
    arr.forEach(function (no, i) {
        ...
        if (DebugON) console.log("pompe " + nom + " state " + no["state"]);
        if (p[nom] && no["volume"]) {
            ...
            if (p[nom]) p[nom].changeState(no["state"]); /// hack pour pompe a vide
        }
    });
}
function gdc() {
    // verfie gallonstotal debit et coulee
...
function scanvalve(arr) {
    arr.forEach(function (no, i) {
        ...
        if (v[nom]) v[nom].changeEtat(no["position"]);
    });
}
...
function scanosmose(arr) {
    ...
    if (!LitCookie) {
        if (getCookie("tableau") !== "") {
            ...
            if (
                tableauLect[0][1] == osmose.PC_Conc &&
                tableauLect[0][2] == osmose.Total_GPH
            ) {
    ...
    ligne[0] = new Date(osmose.startStopTime * 1000)
    ...
    function ecritTableau() {
        for (i = 1; i <= tableauLect.length; i++) {
            //reecrit le tableau
            for (j = 1; j <= 7; j++) {
    ...
    if (osmose.state == 1) {
        osmose.stateValue = $("#stateValue").html("ON");
...
function calcule(station) {
    if (DebugON) console.log("debut aller chercher " + station);
    var resultat2 = "";
...
function oldcalcule(station) {
    ...
    $.ajax({
        url:
            "http://api.wunderground.com/api/880c2ea0b000d306/conditions/q/pws:" +
            station +
            ".json",
        dataType: "jsonp",
    ...
    $.ajax({
        ...
        success: function (parsed_json) {
            try {
                ...
                //var temp_c = 5 ;
                var TempStd = [
                    0, 104, 100, 112, 123.9, 120, 117.7, 114.4, 113.8,
                ]; // temp vs pression std
                var LabelStd = [
                    "<u>Pression</u><br>mba<br>inHg",
                    "<u>Sirop d’érable</u><br>Celsius<br> Fahrenheit",
                    "Reference<br>eau",
                    "Beurre<br> d’érable",
                    "Sucre<br> granulé",
                    "Sucre<br> très dur",
                    "Sucre<br> dur",
                    "Tire d’érable<br>ou sucre mou",
                    "Tire d’érable<br> sur neige",
                ];
...
function Pompe(draw, nom, diam, x, y, couleur) {
    ...
    this.textnomPompe = draw
        .plain(nom)
        .font(MarioLaPolice) //  {family: 'Helvetica', size: 20})
    ...
    this.changeState = function (state, capacity, duty, volume) {
        ...
        if (state === true) {
            if (!this.dateDepartPompe) {
                ...
                if (
                    Date.now() - this.dateDepartPompe > MaxSecPompeOn &&
                    volume
                ) {
                    // hack pour ne pas toucher au vacuum
                    this.des(ORANGE);
        ...
        var toffset = 0; //-20
        if (volume) {
            // hack pas elegant pour pompe a vide  ... pas de texte  pas de volume  ...
...
function Reservoir(draw, nom, diam, x, y, couleur) {
    ...
    this.nom = nom;
    var dvide = ["M", x, y, "h", 10, "v", 10].join(" ");
    ...
    this.changePourtour = function (pourcent) {
        ...
        this.c = draw
            .circle(this.diam)
            .fill({ color: this.couleur, opacity: 0.4 })
            .cx(this.x)
            .cy(this.y)
            .stroke({
...
function res_gallons(reservoirs, arr, niv, total) {
    ...
    arr.forEach(function (key, index) {
        if (!isNaN(reservoirs[key].gallons.toFixed(0))) {
            niv[index].push({
                x: newDateString(0),
                y: reservoirs[key].gallons.toFixed(0),
            });
        } else {
