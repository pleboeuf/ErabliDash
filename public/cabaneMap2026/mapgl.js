...
function scanpoint(datagpx, pointsV, pointsT) {
    ...
    gpx.waypoints.forEach(function (way) {
        ...
        if (way.name[0] === "T") pointsT.push([way.name, [way.lon, way.lat]]);
    });
}
...
function openSocket() {
    ...
    websocket.onmessage = function (msg) {
        if (DebugON) console.log("Socket onmessage.", msg.data);
...
function scanvacuum(arrlocal) {
    ...
    arrlocal.forEach(function (no, i) {
        ...
        if (typeof no.temp !== "undefined") temp = no.temp;
...
function testScanvacuum() {
    arr = Object.keys(lvac);

    if (DebugON) console.log("test scanvacuum: nb vacuum " + arr.length);
    ...
    arr.forEach(function (nom) {
        ...
        var datelect = new Date(Date.now());
        ("--"); //Date.now();
        temp = 99;
        //                   console.log("after before ",JSON.stringify(lvac[nom]));
        if (typeof lvac[nom] !== "undefined") {
            lvac[nom].update(-14 - j++, datelect, temp);
        }
        //           console.log("after update ",JSON.stringify(lvac[nom]));
    });
}
...
function LV(nom) {
    ...
    function isOffline(newlec, anciennelec) {
        ...
        if (isNaN(a) && nd - n < MAXDELAI) {
            //Ier lect si now-lect < delai  online
            return false;
        } else if (isNaN(a) && nd - n >= MAXDELAI) {
            //Ier lect si now-lect >= delai  offline
            return true;
        }
        if (nd - a > MAXDELAI) return true; // delai dépassé
        if (n - a > MAXDELAI) {
            // normal deux lectures
    ...
    this.update = function (vacuum, datelecture, temperature) {
        if (typeof datelecture !== "undefined") {
            // bail out si pas de date
            if (
                isLater(datelecture, this.datelecture) &&
                !isOffline(datelecture, this.datelecture)
            ) {
                // une mise à jour
                if (DebugON)
                    console.log(
    ...
    this.traceTemp = function (nomT, pt) {
        ...
        var geoline = createMarqueurTemp(
            this.nomTemperature,
            99,
            d,
            this.ptTemp,
    ...
    this.findColor = function () {
        ...
        if (this.vacuum === 99) return "blue"; // offline
        var test = myFixed(this.vacuum, 1);
        return findColorRgba(test);
        ////
        /* var color;
        if (test >= -14) {
            color = 'red';
...
function loadLineAutres(lignes, color) {
    ...
    lignes.forEach(function (line) {
        var nom = line[0];
        var geoline = createLigne(nom, line[1], color, 0, 0, 4);
...
function loadMarqueurV(pts) {
    ...
    pts.forEach(function (pt) {
        ...
        if (lvac[nomLV]) lvac[nomLV].traceVac(nom, pt[1]);
    });
}
function loadMarqueurT(pts) {
    ...
    pts.forEach(function (pt) {
        ...
        if (lvac[nomLV]) lvac[nomLV].traceTemp(nom, pt[1]);
    });
}
function nomT(str) {
    var l = str[1];
...
dtcurl = "http://boilerhouse.ddns.net:3300";
...
//
//
function createLigne(nom, lines, color, vacuum, datelecture, size) {
    var myopacity = 0.8;
...
function dateToHour(lect) {
    ...
    if (isNaN(d)) return lect; // retour le texte si c'est pas une date
...
function createMarqueurTemp(nom, temp, datelect, pts) {
    var text = " ";
    if (temp !== 99) text = myFixed(temp, 1);
...
function addClick(nom) {
    map.on("click", nom, function (e) {
        ...
        var description =
            ...
            nom +
            " " +
            e.features[0].properties.valeur +
            " " +
            e.features[0].properties.HeureLecture.toLocaleString() +
            "</h2>";
