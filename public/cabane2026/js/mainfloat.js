...
$('input[type="text"]').change( function () {
    ...
    FFACTOR = $('#testInput').val() * 1.0 ;
    document.cookie = "ffactor=" + FFACTOR;
    calculeBrun("brun");
    //show the text input value in the UI
  //  $('#message span').html(texInputValue);
  });
...
function go(prod) {
    if (ff=getCookieValue("ffactor")){
        FFACTOR=ff*1.0; // conv nombre
       
       // document.cookie = "ffactor=" + FFACTOR*2.0;}
    }
        else 
        document.cookie = "ffactor=" + FFACTOR;

    $('input[type="text"]').val(FFACTOR);  // met FF dans le tableau   cookie ou pas
    if (prod) {
...
//        spin();
  ...
  //      popup();
    ...
    {
        setInterval(chargelecturetest, 1000);  //refresh en millisec 10 min 600000
    }
...
function spin() {
    ...
    $("#spinner").spinner({
        ...
        option: "mouseWheel",
        stop: function (e, ui) {
            FFACTOR=$(this).spinner('value')*1.0; // con en nombre
            //change_brix();
            calcule();
            if (DebugON)
...
//    var myDevices = [];
    ...
    websocket.onopen = function (evt) {
        console.log('Socket opened. '+ wsUri(""), "dashboard-stream"); // fix because il y a un bug dans le code qui fait que onopen est appelé plusieurs fois et ca ouvre plusieurs socket et ca fait n'importe quoi ...  du coup j'ai mis un compteur dans onerror pour limiter les dégâts
    };
...
function wsUri(path) {
  ...
  return "ws://boilerhouse.ddns.net:3300/" ;
 //  return'ws://127.0.0.1:8088/';
    if(l.port == 8080) return ('ws://127.0.0.1:8088/');
...
function oldparseEvent(data) {
    ...
    var res = [];
    var myDevices = _.pluck(data.devices, 'lastEventSerial');
    if (i++ > 0) {
...
if (DebugON)
    ...
    if (res !== undefined && res.length != 0) {
        res.forEach(function (myres)
        {
            ...
            var particule = _.findWhere(data.devices, tata);
//                $("a").append( JSON.stringify(particule) + "<br>");
            if (particule) {
                var nom_device = _.clone({device: particule.name});
                //_.pluck(data.tank,'lastEventSerial');
                var d = _.findWhere(data.devices, _.clone({name: particule.name}));
                if (d)
                    m_devices.push(d);
                var t = _.findWhere(data.tanks, nom_device);
                if (t)
                    m_tanks.push(t);
                var v = _.where(data.valves, nom_device);
                if (v.length > 0) {
                    m_valves.push(v);
                    m_valves = _.flatten(m_valves);
                }
                var vu = _.findWhere(data.vacuum, nom_device);
                if (vu)
                    m_vaccuum.push(vu);
                var p = _.findWhere(data.pumps, nom_device);
                if (p)
                    m_pumps.push(p);
                var o = _.findWhere(data.osmose, nom_device);
                if (o)
                    m_osmoses.push(o);
...
function updateOsmoseDernMAJ(arr){
	...
	$('#majValue').html(delta.toLocaleTimeString());
	//conv date en locale timezone
	/// osmose.lastUpdatedAt = d.toLocaleTimeString();
	//
	//const d = new Date(45000-gmt);

	//console.log(delta.toLocaleTimeString())
	...
	osLabel  =                  
     {
       ...
       "TempsOperEnCour": "En cours depuis ",   
...
// conv date en mm:ss
osmose.TempsOperEnCour = sToMS(osmose.TempsOperEnCour);
osmose.TempsSeq1234= sToMS(osmose.TempsSeq1234 );     
osmose.TempsSeq4321= sToMS(osmose.TempsSeq4321 );     
osmose.TempsDepuisLavage= sToMS(osmose.TempsDepuisLavage );     
osmose.startStopTime= sToMS( osmose.startStopTime);    
...
if(osmose.alarmNo == 4) {
	$("#TempsDepuisLavageLabel").css("background","red");
	$("#TempsDepuisLavageValue").css("background","red");
	
	}else{

		$("#TempsDepuisLavageLabel").css("background","#c2d6c2");
		$("#TempsDepuisLavageValue").css("background","#c2d6c2");
	} 
...
function calcule(station){
    var ligne1= calculeBrun("bru");
 //   var ligne1= calculeBrom("bru");
  //  $('#Pression_temperature').html(ligne1+"<br>+ligne2");   // met a jour le boutton Pression_temperature dans html

}

function calculeBrun(station) {
    // nouvelle station meteo brunelle 
    // https://api.ecowitt.net/api/v3/device/real_time?application_key=F42F5FE57D1A74C4106E87E13E969E9A&api_key=7174a7d92422&mac=24:D7:EB:E9:EC:96&call_back=all
    if (DebugON)
        console.log("debut aller chercher " + station);
    var resultat2 = "";
    // https://api.ecowitt.net/api/v3/device/real_time?
    // application_key=F42F5FE57D1A74C4106E87E13E969E9A&
    // api_key=88e61256-986c-4c46-940e-7174a7d92422&
    // mac=24:D7:EB:E9:EC:96&call_back=all
// https://api.ecowitt.net/api/v3/device/real_time?application_key=F42F5FE57D1A74C4106E87E13E969E9A&api_key=88e61256-986c-4c46-940e-7174a7d92422&mac=24:D7:EB:E9:EC:96&call_back=all
// https://api.ecowitt.net/api/v3/device/real_time?application_key=F42F5FE57D1A74C4106E87E13E969E9A&api_key=7174a7d92422&mac=24:D7:EB:E9:EC:96&call_back=all

    var application_key="F42F5FE57D1A74C4106E87E13E969E9A";
    var api_key="88e61256-986c-4c46-940e-7174a7d92422";
    var urlBrunelle= "https://api.ecowitt.net/api/v3/device/real_time?application_key="+application_key+"&api_key="+api_key+"&mac=24:D7:EB:E9:EC:96&call_back=all"; 
  /*  const options = {
      
        month: "numeric",
        day: "numeric",
      };
    var today = new Date().toLocaleDateString("fr-Ca",options);*/
    const options = {
        timeStyle: "short",
      };
    var today = new Date().toLocaleTimeString()

console.log("url meteo "+urlBrunelle);
    try {
        $.when($.ajax(urlBrunelle).done(function (meteo) {
            var temp_c = FtoC(meteo.data.outdoor.temperature.value*1.0); //": "38.7"  Fahr
            var pressure_inhg = meteo.data.pressure.relative.value*1.0;  //inHg   facteur 3,386
            var pressure_kpa = meteo.data.pressure.relative.value * 3.386388;  //inHg -> kpa  facteur 3.386388
            var ebulition = TempEbulition(pressure_kpa );  // on recoit en hPa div pour kPa
        ...
        //            temp_c.toFixed(1) + " oC </br>" +
         ...
         /*
         resultat2 = `
         ${today.substring(0,5)} ${pressure_inhg.toFixed(2)} poHg ${(pressure_kpa).toFixed(2)} kpa ${temp_c.toFixed(1)} &degC 
         ${ebuSiropF.toFixed(1)} &#8457;  <b>corr. ${FFACTOR}      ${(ebuSiropF+FFACTOR).toFixed(1)} &#8457 </b>` ;
        */
        resultat2 = `
         ${today.substring(0,5)} ${pressure_inhg.toFixed(2)} poHg ${(pressure_kpa).toFixed(2)} kpa<br>Text ${temp_c.toFixed(1)} &degC 
         Pt ebul ${ebuSiropF.toFixed(1)} &#8457;  ` ;
         res3= `   <b>${(ebuSiropF+FFACTOR).toFixed(1)} &#8457 </b>` ;
            $('#Pression_temperature1').html(resultat2);
        //    $('#Pression_temperature2').val(FFACTOR);
            $('#Pression_temperature2 span').html(res3);
        /*
         resultat2 = today +" "+pressure_inhg+" "+ temp_c.toFixed(1) + "&deg " + (pressure_kpa).toFixed(1) + "kpa " + 
         "  " + ebuSiropF.toFixed(1)+ "&#8457; " + FFACTOR + " "+(ebuSiropF+FFACTOR).toFixed(1); 
            $('#Pression_temperature1').html(resultat2);   // met a jour le boutton Pression_temperature dans html
            */
        }));
    } catch (err) {
...
function calculeBrom(station) {
    ...
    try{
        $.when($.ajax(url2).done(function (meteo) {
            ...
            //        temp_c.toFixed(1) + " oC </br>" +
            //        +ebuSiropF.toFixed(1) + " oF ";
            resultat2 = today +" "+ temp_c.toFixed(1) + "oC " + pressure_mb.toFixed(0) + "mb " + 
                    "  " + ebuSiropF.toFixed(1)+ "oF";
            $('#Pression_temperature2').html(resultat2);   // met a jour le boutton Pression_temperature dans html
...
// on affiche quand meme  ...because possible appel par la fonction de mise a jour du brix
 ...
 //       console.log("reservoir " + this.nom + " pourcent " + pourcent + " contient " + contient + " datelecture " + datelecture);   
        ...
        /*        this.textP = draw.plain(parseInt(this.pourcent, 10) + "%").cx(this.x).cy(this.y + toffset);
         if (contient > 0) this.textP2 = draw.plain( contient).cx(this.x).cy(this.y + toset + 15 );
         */
//        this.textP.text(parseInt(this.pourcent, 10) + "%").move(this.x,(this.y + toffset));
